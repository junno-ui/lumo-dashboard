...
import { registerDashboardStats as stats, features, recentSignups } from '@/modules/auth/data/auth'
</script>
  ...
  <div class="mt-auto pt-10 w-full">
    <div class="relative max-w-3xl xl:max-w-5xl 2xl:max-w-6xl mx-auto">
        ...
        <div class="p-5 xl:p-8 2xl:p-10 space-y-4 xl:space-y-6">
          ...
          <div class="rounded-xl border border-neutral-200/80 bg-white p-4 xl:p-6
            ...
            <div class="flex items-center justify-between">
              ...
              <UBadge size="xs" color="primary" variant="soft">{{ stats.step }}</UBadge>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-5 gap-3 xl:gap-5">
            ...
            <div class="sm:col-span-2 rounded-xl border border-neutral-200/80 bg-white p-4 xl:p-6
                   ...
                   dark:border-white/10 dark:bg-white/5 dark:hover:border-white/20 dark:hover:shadow-black/40">
              <div class="flex items-center justify-between">
                <p class="text-[11px] font-medium text-neutral-700 dark:text-white/80">{{ stats.planName }}</p>
                <UBadge size="xs" color="success" variant="soft">Free</UBadge>
              </div>

              <p class="mt-2 text-2xl font-semibold text-neutral-900 dark:text-white">{{ stats.price }}</p>
              ...
              <div class="mt-3 rounded-lg bg-gray-50 dark:bg-white/5 p-3">
                <div class="flex items-center justify-between">
                  ...
                  <p class="text-[10px] font-medium text-neutral-700 dark:text-white/80">{{ stats.storage }}</p>
                </div>
            ...
            <div class="sm:col-span-3 rounded-xl border border-neutral-200/80 bg-white p-4 xl:p-6
              ...
              <div class="flex items-center justify-between">
                ...
                <UBadge size="xs" color="neutral" variant="soft"
                  class="bg-gray-100 text-neutral-700 dark:bg-white/10 dark:text-white/80">
                  Included
                </UBadge>
              </div>
              ...
              <ul class="mt-3 space-y-2 text-[11px]">
                <li v-for="feature in features" :key="feature" class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-lg bg-gray-100 dark:bg-white/10 flex items-center justify-center">
                    <UIcon name="mdi:check-circle-outline" class="w-4 h-4 text-neutral-700 dark:text-white/80" />
                  </span>
                  <span class="text-neutral-700 dark:text-white/80">{{ feature }}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- Bottom: table -->
          <div class="rounded-xl border border-neutral-200/80 overflow-hidden
            ...
            <div class="divide-y divide-neutral-200/80 dark:divide-white/10 text-[11px]">
              <div v-for="signup in recentSignups" :key="signup.email"
                class="px-4 py-3 flex items-center justify-between transition-colors duration-200 hover:bg-gray-50/70 dark:hover:bg-white/5">
                <div class="flex items-center gap-2">
                  <div class="w-7 h-7 rounded-lg bg-gray-100 dark:bg-white/10 flex items-center justify-center">
                    <UIcon name="mdi:account-outline" class="w-4 h-4 text-neutral-600 dark:text-white/70" />
                  </div>
                  <div>
                    <p class="font-medium text-neutral-900 dark:text-white">{{ signup.email }}</p>
                    <p class="text-[10px] text-neutral-500 dark:text-white/60">{{ signup.plan }}</p>
                  </div>
                </div>
                <UBadge size="xs" :color="signup.status === 'Verified' ? 'success' : signup.status === 'Pending' ? 'warning' : 'primary'"
                  variant="soft">{{ signup.status }}</UBadge>
              </div>
            </div>
          </div>
        </div>
      ...
      <UCard class="relative mt-4 md:mt-0 md:absolute md:-top-8 md:left-5 w-full md:w-44 rounded-2xl
        ...
        <p class="mt-1 text-base font-semibold">{{ stats.setupScore }}</p>
